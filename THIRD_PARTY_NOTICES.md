...
all models, while `vq model fetch-speech` downloads only these speech assets,
into the user's Hugging Face and `vq` runtime caches. The native runtime release
archive is verified against a pinned SHA-256 digest before its
`llama-funasr-sensevoice` binary is installed in the cache.
